...
def get_default_gateway_windows():
    ...
    try:
        # 方法1: 使用route print命令
        result = subprocess.run("route print -4", shell=True,
                              capture_output=True, text=True,
                              encoding='utf-8', errors='ignore')
        ...
        for line in lines:
            ...
            elif ("默认网关" in line or "default gateway" in line.lower()) and ":" in line:
                gateway = line.split(':')[1].strip()
                # 清理网关地址
                gateway = re.sub(r'[^0-9.]', '', gateway)
                if gateway and len(gateway.split('.')) == 4 and gateway not in gateways:
                    gateways.append(gateway)
                    print(f"🔍 ipconfig发现网关: {gateway} (接口: {current_interface})")
        
        # 方法3: 使用netsh命令
        result = subprocess.run("netsh interface ipv4 show config", shell=True, 
                               capture_output=True, text=True, encoding='utf-8', errors='ignore')
        lines = result.stdout.split('\n')
        current_interface = None
        ...
        for line in lines:
            line = line.strip()
            if "配置" in line and "接口" in line:
                current_interface = line.split("'")[1] if "'" in line else line
            elif ("默认网关" in line or "default gateway" in line.lower()) and ":" in line:
                gateway = line.split(':')[1].strip()
                gateway = re.sub(r'[^0-9.]', '', gateway)
                if gateway and len(gateway.split('.')) == 4 and gateway not in gateways:
                    gateways.append(gateway)
                    print(f"🔍 netsh发现网关: {gateway} (接口: {current_interface})")
        
        # 方法4: 使用wmic命令
        try:
            result = subprocess.run("wmic nicconfig get DefaultIPGateway", shell=True,
                                  capture_output=True, text=True, encoding='utf-8', errors='ignore')
            lines = result.stdout.split('\n')
            for line in lines:
                if "{" in line and "}" in line:
                    # 提取网关IP，格式如: {192.168.1.1}
                    match = re.search(r'\{([0-9.]+)\}', line)
                    if match:
                        gateway = match.group(1)
                        if gateway not in gateways:
                            gateways.append(gateway)
                            print(f"🔍 wmic发现网关: {gateway}")
        except:
            pass
        ...
        if result.returncode == 0:
            ...
            for line in lines:
                ...
                if len(parts) >= 2:
                    ...
                    if gateway and len(gateway.split('.')) == 4 and gateway not in gateways:
                        gateways.append(gateway)
                        print(f"🔍 netstat发现网关: {gateway}")
        
        # 方法3: 使用route命令
        result = subprocess.run("route -n | grep '^0.0.0.0'", shell=True,
                               capture_output=True, text=True, encoding='utf-8', errors='ignore')
        if result.returncode == 0:
            lines = result.stdout.split('\n')
            for line in lines:
...
def get_default_gateway():
    """获取默认网关 - 跨平台版本，返回所有发现的网关"""
    print("🔄 正在自动检测网关...")
    
    if platform.system() == "Windows":
        gateways = get_default_gateway_windows()
    else:
        gateways = get_default_gateway_linux()
    
    if gateways:
        print(f"✅ 共发现 {len(gateways)} 个网关")
        return gateways
    else:
        print("❌ 无法自动检测网关")
        return []

def select_gateway_automatically(gateways, interface=None):
    """自动选择最合适的网关"""
    if not gateways:
        return None
    ...
    for gateway in gateways:
        print(f"   测试网关 {gateway}...", end="")
        mac = get_mac(gateway, interface)
        if mac:
            print(" ✅ 在线")
            print(f"      MAC地址: {mac}")
            return gateway, mac
    ...
    return None, None
def select_gateway_manually(gateways, interface=None):
    """手动选择网关"""
    print("\n🎯 请选择网关:")
    print("="*40)
    ...
    if gateways:
        ...
        for i, gateway in enumerate(gateways, 1):
            print(f"{i}. {gateway}")
        print(f"{len(gateways)+1}. 手动输入网关地址")
    else:
        print("1. 手动输入网关地址")
    
    try:
        if gateways:
            choice = input(f"\n请选择网关 (1-{len(gateways)+1}, 默认1): ").strip()
            if not choice:
                choice = "1"
        else:
            choice = "1"
        ...
        if choice.isdigit():
            ...
            if 1 <= choice_num <= len(gateways):
                ...
                print(f"✅ 已选择网关: {selected_gateway}")
                
                # 测试网关连通性
                print(f"测试网关 {selected_gateway}...")
                gateway_mac = get_mac(selected_gateway, interface)
                if gateway_mac:
                    print(f"✅ 网关 {selected_gateway} 可用，MAC: {gateway_mac}")
                    return selected_gateway, gateway_mac
                else:
                    print(f"❌ 无法连接到网关 {selected_gateway}")
                    retry = input("是否重试? (y/N): ").strip().lower()
                    if retry == 'y':
                        return select_gateway_manually(gateways, interface)
                    else:
                        return None, None
            elif choice_num == len(gateways) + 1 or (not gateways and choice_num == 1):
                # 手动输入
                return input_gateway_manually(interface)
            else:
                print("❌ 无效选择")
                return select_gateway_manually(gateways, interface)
        else:
...
def input_gateway_manually(interface=None):
    """手动输入网关地址"""
    print("\n🔧 手动输入网关地址")
    print("="*30)
...
def choose_gateway_selection_method(interface=None):
    """选择网关获取方式"""
    print("\n🌐 网关选择方式")
    print("="*40)
    print("1. 自动检测并选择网关 (推荐)")
    print("2. 手动选择网关")
    print("3. 手动输入网关地址")
    ...
    try:
        ...
        if choice == "1":
            ...
            if gateways:
                ...
                # 自动选择第一个可用的网关
                selected_gateway, gateway_mac = select_gateway_automatically(gateways, interface)
                if selected_gateway:
                    print(f"\n🎯 自动选择网关: {selected_gateway}")
                    return selected_gateway, gateway_mac
                else:
                    print("❌ 所有自动发现的网关都不可用")
                    print("切换到手动选择模式...")
            ...
            print("❌ 无效选择，使用自动检测")
            return choose_gateway_selection_method(interface)
...
def run_arp_spoofer():
    ...
    # 显示网络信息
    local_ip, detected_gateways = get_network_info(interface)
    if not local_ip:
    ...
    print("\n🌐 网关配置")
    print("="*30)
    GATEWAY_IP, GATEWAY_MAC = choose_gateway_selection_method(interface)
